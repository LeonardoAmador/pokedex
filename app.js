...
const getPokemonsType = async (pokeApiResults) => {
  const fulfilled = await getOnlyFullfilled({ 
    arr: pokeApiResults, func: result => fetch(result.url) 
  });
...
const getPokemonsImgs = async (ids) => {
  const fulfilled = await getOnlyFullfilled({
    arr: ids, func: id => fetch(`./assets/img/${id}.png`)
  });
...
const getPokemons = async () => {
  try { 
    ...
    if (!response.ok) {
      throw new Error('It wasnt possible to get the informations.');
    }
